...
labels = pd.read_csv(directory + '\\labels.csv') # CSV with filename, and breed.
...
learning_rate = 0.001
iters = 10  # Iterations before checking validation
epochs = 100 # Overall generations
...
def train_model(learning_rate):
    ...
    start = time.time()
    hidden_layer1_size = breeds_num * 4
    hidden_layer2_size = int(hidden_layer1_size * 0.5)
    hidden_layer3_size = int(hidden_layer2_size * 0.5)
    ...
    weights2 = tf.Variable(tf.truncated_normal([hidden_layer1_size, hidden_layer2_size], stddev=0.1), name="weights2")
    bias2 = tf.Variable(tf.constant(0.1, shape=[hidden_layer2_size]), name="bias2")
    prediction2 = tf.nn.relu(tf.matmul(prediction1, weights2) + bias2, name="prediction2")

    weights3 = tf.Variable(tf.truncated_normal([hidden_layer2_size, hidden_layer3_size], stddev=0.1), name="weights3")
    bias3 = tf.Variable(tf.constant(0.1, shape=[hidden_layer3_size]), name="bias3")
    prediction3 = tf.nn.relu(tf.matmul(prediction2, weights3) + bias3, name="prediction3")

    weights4 = tf.Variable(tf.truncated_normal([hidden_layer3_size, breeds_num], stddev=0.1), name="weights4")
    bias4 = tf.Variable(tf.constant(0.1, shape=[breeds_num]), name="bias4")
    prediction4 = tf.nn.relu(tf.matmul(prediction3, weights4) + bias4, name="prediction4")

    # Loss function
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prediction4, labels=answers), name="loss")
    train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss)

    correct_prediction = tf.equal(tf.argmax(prediction4, 1), tf.argmax(answers, 1))
    ...
    for _ in range(epochs):
        ...
        print(f'After {int(time.time() - start)}s, accuracy avg {avg_acc}, loss avg {avg_loss}')
    ...
    try:
        ...
        print("No pictures of bonito.")

    plot(epochs, accuracy_avg_tracker, loss_avg_tracker)
    return prediction
